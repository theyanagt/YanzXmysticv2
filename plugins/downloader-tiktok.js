...
let handler = async (m, { conn, text, usedPrefix }) => {
	...
	await m.reply('_In progress, Follow Tiktok Sy @mychicz_')
	let buttons = [{ buttonText: { displayText: 'Audio' }, buttonId: `${usedPrefix}tomp3` }]
	conn.sendMessage(m.chat, { video: { url: res.link }, caption: meta?.description || res?.description, footer: await shortUrl(), buttons }, { quoted: m })
